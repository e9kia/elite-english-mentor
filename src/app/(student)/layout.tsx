...
// Student-facing shell — premium nav + global heartbeat + Ali Jitam signature
...
export default async function StudentLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 elite-card border-b border-gold/5">
        <div className="mx-auto max-w-7xl px-6 h-16 flex items-center justify-between">
          <Link href="/dashboard" className="flex items-center gap-3 hover:opacity-80 transition-opacity group">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-br from-primary/20 to-gold/10 border border-primary/20 flex items-center justify-center group-hover:scale-105 transition-transform">
            ...
            <div>
              ...
              <p className="text-[10px] text-gold/50 leading-none mt-0.5 font-bold tracking-widest uppercase">4,000 Words</p>
            </div>
          </Link>
      ...
      {/* ═══ ELITE FOOTER ═══ */}
      <footer className="border-t border-gold/5 py-12 elite-card">
